...
def smiles2graph_wrapper(smiles_list):
    ...
    try:
        ...
        tgt_graph = smiles2graph(tgt,bfs=True)
    except:
        return "error"
...
def smiles2graph(smiles_string, bfs=True):
    ...
    #         node_edge_seq.append(G.nodes[bfsseq[nodeindx]]['node']) # random begin index
    #         #prevnum = min(max_prev,nodeindx)
    #         gap =0
    #         for j in range(0,nodeindx):
    #             #check i-1, i-2...i-n node
    #             check_node_idx = nodeindx-(j+1)
    #             try:
    #                 hasedge = int(G[bfsseq[check_node_idx]][bfsseq[nodeindx]]['edge'])
    #             except:
    #                 hasedge = None
    #                 gap +=1
    #             if j<max_prev and hasedge:
    #                 if gap>0:
    #                     node_edge_seq.append(506+gap)
...
class UsptoDataset(InMemoryDataset):
    def __init__(self, root = 'data/uspto-full-and-distill', smiles2graph = smiles2graph, transform=None, pre_transform = None, smiles2graph_wrapper = smiles2graph_wrapper):
        '''
        ...
        for j in data_df:
            ...
            with Pool(processes = 70) as pool:
                ...
                for idx, graph in tqdm(enumerate(iter),total = len(smiles_list)):
                    ...
                    #     data.y_frags = tgt_graph['frags']
                    #     data.y_node_attr = tgt_graph['g_node_attr']
                    #     y_graph_list.append(tgt_graph['node_edge_seq'])
                    #     data.reverse = 1
                    #     np_data_list.append(data)
    ...
    def len(self):
        return 96026+975838+96067
        #return 96026+1422976+96067
        #return 852000
        #return 4000
        #return 960923 #960923
        #return 960900
        #961096
        #return 400000+5000
        #return 45000
    ...
    def get_idx_split(self):
        ...
        #return 100,3900,4000
        #return 20,0,0
        #return 60000,0,0
        return 96026,96026+975838,96026+975838+96067
        #return 492,0,0
        #return 9133,9133,9133
        #return 5000,45000,50000
        #return 111098,111098,111098
        #return 100000,500299,500299
        #return 0,40000*5,40000*5+5000
        #return 5000,0,0
        #return  self.testindx , self.trainindx , self.valindx
    def get_dic(self):
        return self.dic
        ...
        if y[i]!=0 and y[i]> 472+4 and y[i]<=(504+4):
            ...
            for k in range(i+1):
                ...
                if NodeEdge != 0 and NodeEdge <=472+offset:
                    ...
                    node_indx+=1
            # sparse = sp.dok_matrix(lpe_bias[i])
            # print(lpe_bias[i].nbytes)
            # print(sys.getsizeof(sparse))
            # lpe_bias_sparse.append(sparse)
        else:
...
### Preprocess generate padding mask/graph_seq/lpe/centrality
def g2graph_seq(G,frags,random=True):
    ...
    #rng.shuffle(np.array(full_frag,dtype=object)) # shuffle molecules order
    #print(full_frag)
    random = False
    random_type = 0
    if random:
        random_type = rng.choice([0,2,3])
        rng.shuffle(np.array(full_frag,dtype=object)) 
        #random_type = rng.choice([0,1])
    # if random_type==3:
    #     rng.shuffle(np.array(full_frag,dtype=object)) 
...
def preprocess_item(item, noise=False,random=True):
    ...
    #y_adj, y_indegree 
    K = y_num_nodes
    #y_adj = torch.zeros([K, K], dtype=torch.bool)
    #y_adj[y_edge_index[0, :], y_edge_index[1, :]] = True
    #y_in_degree = y_adj.long().sum(dim=1).view(-1)
    #y_central, lpe, y_attn_bias========================

    #lpe_eigenval = lpe_input = central_input = np.zeros([1])
    lpe_input,lpe_eigenval, central_input = make_laplacian_cent_attn_bias(30,y)  

    y_attn_bias = torch.zeros([M+1 , M+1], dtype=torch.float)
    
#     y_attn_bias = torch.zeros([K, K], dtype=torch.float) # no graph token
    ...
    start = torch.tensor([505+offset],dtype=torch.long)
    end = torch.tensor([506+offset],dtype=torch.long)
    y=torch.cat((start,y))
    # if begin_indx == 0:
    #     y=torch.cat((start,y))
    # elif begin_indx==1:
    #     y=torch.cat((start+1,y))
    # elif begin_indx==2:
    #     y=torch.cat((start+2,y))
    # elif begin_indx==3:
    #     y=torch.cat((start+3,y))
    y = torch.cat((y,end))
    ...
    # new 
    # item.y = y.clone().detach()
    item.central_input = central_input
    item.lpe_input = lpe_input
    item.lpe_eigenval = lpe_eigenval
    #item.y_num_nodes = y_num_nodes
    item.y_attn_bias = y_attn_bias
    item.y = y
    #item.begin_indx = begin_indx
    
    return item

class dataloader(UsptoDataset):
    def __init__(self, root = 'data/uspto-full-direct', random = True):
        super().__init__(root=root)
        self.y = pickle.load(open(self.root+'/processed/y_graph_inputs.pkl', 'rb'))
        self.random = random

    def process(self):
        super(dataloader, self).process()
    ...
    def __getitem__(self, idx):
        if isinstance(idx, int):
            ...
            item.idx = idx
            item.y = self.y[self.indices()[idx]]
            
            return  preprocess_item(item, noise=True,random=self.random)
        else:
